...
export async function loadLogoDataUrl() {
  ...
  const asset = Asset.fromModule(require('../assets/logo-nehoc.jpeg'));
  await asset.downloadAsync();
  if (!asset.localUri) return '';
  const b64 = await FileSystem.readAsStringAsync(asset.localUri, {
    encoding: FileSystem.EncodingType.Base64,
  });
  cachedLogo = `data:image/jpeg;base64,${b64}`;
  return cachedLogo;
}
...
export async function buildReportHtml(site: Site): Promise<string> {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    *{box-sizing:border-box} body{margin:0;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;color:#181818;background:#fff}
    .report-page{width:210mm;min-height:297mm;padding:12mm 13mm 10mm;position:relative;display:flex;flex-direction:column;page-break-after:always}
