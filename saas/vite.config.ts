...
function attachIcsFile(server: ViteDevServer | PreviewServer) {
  server.middlewares.use((req, res, next) => {
    const raw = req.url || '';
    if (!raw.startsWith('/nehoc-pose.ics')) {
      next();
      return;
    }
...
function icsFilePlugin(): Plugin {
  return {
    name: 'ics-file',
    configureServer: attachIcsFile,
    configurePreviewServer: attachIcsFile,
  };
}

export default defineConfig({
  plugins: [react(), icsFilePlugin()],
